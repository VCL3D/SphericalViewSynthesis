...
def initialize_weights(model, init = "xavier", pred_bias=None):
    init_func = None
    if init == "xavier":
    ...
    if init_func is not None:
        ...
        if pred_bias is not None:
            list(model.modules())[-1].bias.data.fill_(pred_bias)
...
def initialize_prediction_bias(model, pred_bias=None):
    if pred_bias is not None:
        list(model.modules())[-1].bias.data.fill_(pred_bias)
